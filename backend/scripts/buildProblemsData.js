...
export const PROBLEMS_DATA = [
  {
    ...
    patterns: ["Hashing / Frequency Map", "Two Pointers"],
...
class Solution {
    ...
    stepByStep: [
      { step: 1, title: "Understand the Problem", text: "Find 2 distinct indices in nums whose sum equals target." },
      { step: 2, title: "Identify Input / Output", text: "Input: int[] nums, int target. Output: int[] of 2 indices." },
      { step: 3, title: "Analyze Constraints", text: "n up to 10^4. O(n²) will execute ~10^8 operations (borderline TLE). We need O(n) or O(n log n)." },
      { step: 4, title: "Brute Force Idea", text: "Check every pair with two nested loops. Time: O(n²), Space: O(1)." },
      { step: 5, title: "Identify Bottleneck", text: "Repeated linear scan to find whether target - nums[i] exists in the remainder of the array." },
      { step: 6, title: "Recognize Pattern", text: "Hashing / Complement Lookup pattern converts O(n) search to O(1) average hash lookup." },
      { step: 7, title: "Optimize", text: "Store visited numbers in a HashMap mapping value to index." },
      { step: 8, title: "Implement", text: "Single pass: check map for complement, if absent put nums[i] -> i." },
      { step: 9, title: "Test Edge Cases", text: "Negative numbers, duplicate values summing to target (e.g. [3, 3] target 6), first/last element pairs." },
      { step: 10, title: "Analyze Complexity", text: "Time: O(n) where n is nums.length. Space: O(n) to store up to n elements in map." }
    ],
    testCases: [
      ...
      { input: [[1000000, 500, -1000000], -500000], expected: [], isHidden: true }
    ]
  },
  {
    id: "valid-palindrome",
    ...
    companies: ["Meta", "Microsoft", "Amazon", "Apple", "Spotify"],
...
Given a string \`s\`, return \`true\` if it is a palindrome, or \`false\` otherwise.`,
    ...
    stepByStep: [
      { step: 1, title: "Understand the Problem", text: "Check if characters read the same forwards and backwards, ignoring casing and symbols." },
      { step: 2, title: "Identify Input / Output", text: "Input: String s. Output: boolean." },
      { step: 3, title: "Constraints", text: "Length up to 2 * 10^5. Must run in O(n) time." },
      { step: 4, title: "Pattern Recognition", text: "Two Pointers starting at 0 and n-1 converging inwards." },
      { step: 5, title: "Optimize Memory", text: "Do not create new strings. Use Character.isLetterOrDigit directly on original string." },
      { step: 6, title: "Analyze Complexity", text: "Time: O(n) single pass. Space: O(1) constant pointers." }
    ],
    testCases: [
  ...
  },
  {
    ...
    patterns: ["Kadane's Algorithm", "Dynamic Programming (1D)"],
...
A subarray is a contiguous non-empty sequence of elements within an array.`,
    ...
    stepByStep: [
      { step: 1, title: "Understand Problem", text: "Find largest contiguous sum in integer array with negative and positive values." },
      { step: 2, title: "Identify Bottleneck", text: "O(n²) calculates overlapping subarray sums repeatedly." },
      { step: 3, title: "Key Insight", text: "If current sum becomes negative, starting fresh at nums[i] is strictly superior." },
      { step: 4, title: "State Transition", text: "dp[i] = max(nums[i], dp[i-1] + nums[i])." },
      { step: 5, title: "Space Optimization", text: "Only need scalar variable currentMax instead of full array." }
    ],
    testCases: [
      ...
      { input: [-5, -2, -8, -1, -4], expected: -1, isHidden: true },
  ...
  },
  {
    ...
    patterns: ["Sliding Window", "Hashing / Frequency Map"],
...
class Solution {
    ...
    stepByStep: [
      { step: 1, title: "Understand", text: "Find longest substring (contiguous) containing unique characters." },
      { step: 2, title: "Recognize Pattern", text: "Sliding Window with dynamic boundaries." },
      { step: 3, title: "Window State", text: "Window [left, right] always maintains non-repeating characters." },
      { step: 4, title: "Avoid Backward Jump", text: "Use Math.max(left, map.get(c) + 1) because map may contain characters before left." },
      { step: 5, title: "Complexity", text: "Time: O(n). Space: O(min(m, n)) where m is charset size." }
    ],
    testCases: [
      ...
      { input: "abba", expected: 2, isHidden: true }
    ]
  }
];

fs.writeFileSync("src/data/problemsData.js", "export const PROBLEMS_DATA = " + JSON.stringify(PROBLEMS_DATA, null, 2) + ";\n", "utf8");
console.log("Wrote problemsData.js with rich interactive problems!");
