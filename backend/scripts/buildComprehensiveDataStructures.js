...
- In contrast, linked structures suffer pointer chasing cache misses (often 200+ CPU clock cycles per node dereference).`,
    operationsTable: [
      { operation: "Access by Index", complexity: "O(1)", note: "Direct pointer arithmetic (Base + i * size)" },
      { operation: "Search (Unsorted)", complexity: "O(n)", note: "Linear scan through elements" },
      { operation: "Search (Sorted)", complexity: "O(log n)", note: "Binary search on sorted indices" },
      { operation: "Insert at Beginning", complexity: "O(n)", note: "Must shift all existing elements right by 1" },
      { operation: "Insert at End", complexity: "O(1)*", note: "Amortized constant time; O(n) during resize" },
      { operation: "Insert at Index i", complexity: "O(n)", note: "Shifts (size - i) elements right" },
      { operation: "Delete from Beginning", complexity: "O(n)", note: "Shifts all remaining elements left by 1" },
      ...
      steps: [
        {
          ...
          visualState: [
            { idx: 0, val: 12, label: "Base" },
            { idx: 1, val: 24, label: "Base+4B" },
            { idx: 2, val: 36, label: "Target (O(1))" },
            { idx: 3, val: 48, label: "Base+12B" }
          ]
        },
        {
          ...
          visualState: [
            { idx: 0, val: 12, label: "Kept" },
            { idx: 1, val: 99, label: "New (99)", highlight: "inserted" },
            { idx: 2, val: 24, label: "Shifted →", highlight: "shifted" },
            { idx: 3, val: 36, label: "Shifted →", highlight: "shifted" },
            { idx: 4, val: 48, label: "Shifted →", highlight: "shifted" }
          ]
        },
        {
          ...
          visualState: [
            { idx: 0, val: 12, label: "Copied" },
            { idx: 1, val: 99, label: "Copied" },
            { idx: 2, val: 24, label: "Copied" },
            { idx: 3, val: 36, label: "Copied" },
            { idx: 4, val: 48, label: "Copied" },
            { idx: 5, val: null, label: "Free Slot" }
          ]
        }
      ]
    },
...
- If 'count == value.length', it doubles capacity: '(value.length << 1) + 2'.`,
    operationsTable: [
      { operation: "Access char at index", complexity: "O(1)", note: "s.charAt(i) returns byte/char directly" },
      { operation: "Substring", complexity: "O(k)", note: "Copies k characters into newly allocated String" },
      { operation: "String Concatenation in Loop (+)", complexity: "O(n²)", note: "Anti-pattern! Reallocates and copies every iteration" },
      { operation: "StringBuilder.append()", complexity: "O(1)*", note: "Amortized append to internal character buffer" },
      { operation: "String.indexOf()", complexity: "O(n * m)", note: "Naive substring search; O(n) with KMP/Rabin-Karp" },
      { operation: "String.equals()", complexity: "O(n)", note: "Compares lengths first, then byte-by-byte" }
    ],
    methodsList: [
      {
      ...
      steps: [
        {
          ...
          visualState: [
            { idx: 1, val: "\"a\"", label: "Alloc #1 (1 byte)" },
            { idx: 2, val: "\"ab\"", label: "Alloc #2 (2 bytes)" },
            { idx: 3, val: "\"abc\"", label: "Alloc #3 (3 bytes)" },
            { idx: 4, val: "Total", label: "O(n²) Memory Waste" }
          ]
        },
        {
          ...
          visualState: [
            { idx: 0, val: "'a'", label: "buffer[0]" },
            { idx: 1, val: "'b'", label: "buffer[1]" },
            { idx: 2, val: "'c'", label: "buffer[2]" },
            { idx: 3, val: "null", label: "Unused buffer slot" },
            { idx: 4, val: "Total", label: "O(n) Linear Time" }
          ]
        }
      ]
    },
...
- If a cycle exists, Fast must overlap Slow within the loop in O(n) time.`,
    operationsTable: [
      { operation: "Insert at Head", complexity: "O(1)", note: "newHead.next = head; head = newHead" },
      { operation: "Insert at Tail (with tail ref)", complexity: "O(1)", note: "tail.next = newNode; tail = newNode" },
      { operation: "Delete Head", complexity: "O(1)", note: "head = head.next" },
      { operation: "Delete Node (given prev node)", complexity: "O(1)", note: "prev.next = prev.next.next" },
      { operation: "Search by Value", complexity: "O(n)", note: "Must traverse node pointers sequentially" },
      { operation: "Access by Index i", complexity: "O(n)", note: "Must traverse i nodes from head" }
    ],
    methodsList: [
      {
        name: "addFirst(E e)",
      ...
      steps: [
        {
          ...
          visualState: [
            { idx: 0, val: "[Data: 10 | next] →", label: "Head" },
            { idx: 1, val: "[Data: 20 | next] →", label: "Node 1" },
            { idx: 2, val: "[Data: 30 | next] → null", label: "Tail" }
          ]
        },
        {
          ...
          visualState: [
            { idx: 0, val: "[Data: 5 | next] →", label: "New Head (O(1))", highlight: "inserted" },
            { idx: 1, val: "[Data: 10 | next] →", label: "Old Head" },
            { idx: 2, val: "[Data: 20 | next] →", label: "Node 1" },
            { idx: 3, val: "[Data: 30 | next] → null", label: "Tail" }
          ]
        }
      ]
    },
...
- To find Next Greater Element: iterate right-to-left. Pop all stack elements <= current element. The top of the stack is now the next greater element! Push current element. Total runtime: O(n) because each item enters and leaves the stack at most once.`,
    operationsTable: [
      { operation: "Push", complexity: "O(1)*", note: "Add element to top of stack" },
      { operation: "Pop", complexity: "O(1)", note: "Remove and return top element" },
      ...
      { operation: "isEmpty", complexity: "O(1)", note: "Checks if size == 0" }
    ],
    methodsList: [
      {
        name: "push(E e)",
      ...
      steps: [
        {
          ...
          visualState: [
            { idx: 2, val: 30, label: "Top (Most Recent)", highlight: "inserted" },
            { idx: 1, val: 20, label: "Middle" },
            { idx: 0, val: 10, label: "Bottom" }
          ]
        },
        {
          ...
          visualState: [
            { idx: 1, val: 20, label: "New Top (O(1))", highlight: "shifted" },
            { idx: 0, val: 10, label: "Bottom" }
          ]
        }
      ]
...
- As the window slides, remove indices that have fallen out of the window from the front, and remove smaller elements from the rear. Front of deque always holds the max element!`,
    operationsTable: [
      ...
      { operation: "Peek", complexity: "O(1)", note: "Inspect head element without removing" },
      { operation: "offerFirst / offerLast", complexity: "O(1)*", note: "Insert at either end in Deque" },
      { operation: "pollFirst / pollLast", complexity: "O(1)", note: "Remove from either end in Deque" }
    ],
    methodsList: [
      {
        name: "offer(E e)",
      ...
      steps: [
        {
          ...
          visualState: [
            { idx: 0, val: 10, label: "Front (Leaves Next)" },
            { idx: 1, val: 20, label: "Middle" },
            { idx: 2, val: 30, label: "Rear (Just Arrived)", highlight: "inserted" }
          ]
        },
        {
          ...
          visualState: [
            { idx: 0, val: 20, label: "New Front (O(1))", highlight: "shifted" },
            { idx: 1, val: 30, label: "Rear" }
          ]
        }
      ]
    },
...
- If tree shrinks to <= 6 nodes during deletions, it converts back to a linked list (untreeify).`,
    operationsTable: [
      { operation: "Insert (put)", complexity: "O(1) avg / O(log n) worst", note: "Amortized O(1); treeified buckets take O(log n)" },
      { operation: "Lookup (get)", complexity: "O(1) avg / O(log n) worst", note: "Computes hash index and checks key.equals()" },
      { operation: "Delete (remove)", complexity: "O(1) avg / O(log n) worst", note: "Unlinks node from bucket chain/tree" },
      { operation: "containsKey", complexity: "O(1) avg", note: "Verifies key presence" },
      { operation: "Resize (Rehash)", complexity: "O(n)", note: "Allocates double capacity and redistributes entries" }
    ],
    methodsList: [
      {
      ...
      steps: [
        {
          ...
          visualState: [
            { idx: 0, val: "Bucket 0: null", label: "Empty" },
            { idx: 1, val: "Bucket 1: null", label: "Empty" },
            { idx: 2, val: "Bucket 2: [apple → 5]", label: "Placed in Bucket 2", highlight: "inserted" },
            { idx: 3, val: "Bucket 3: null", label: "Empty" }
          ]
        },
        {
          ...
          visualState: [
            { idx: 0, val: "Bucket 0: null", label: "Empty" },
            { idx: 1, val: "Bucket 1: null", label: "Empty" },
            { idx: 2, val: "[apple → 5] → [banana → 9]", label: "Chained Collision", highlight: "shifted" },
            { idx: 3, val: "Bucket 3: null", label: "Empty" }
          ]
        }
      ]
...
- Balanced condition: |height(left) - height(right)| <= 1 for every node.`,
    operationsTable: [
      { operation: "Search (Balanced BST)", complexity: "O(log n)", note: "Halves search space at each branch" },
      { operation: "Search (Degenerate BST)", complexity: "O(n)", note: "Degenerates into a linear linked list" },
      { operation: "Insert / Delete (Balanced)", complexity: "O(log n)", note: "Maintains BST property via tree rotations" },
      { operation: "Traversals (In/Pre/Post/Level)", complexity: "O(n)", note: "Visits every node exactly once" },
      { operation: "Lowest Common Ancestor (LCA)", complexity: "O(h)", note: "h is tree height (O(log n) balanced)" }
    ],
    methodsList: [
      {
      ...
      steps: [
        {
          ...
          visualState: [
            { idx: 0, val: "Root (20)", label: "15 < 20 → Go Left", highlight: "inserted" },
            { idx: 1, val: "Right Subtree (30, 40)", label: "Discarded" }
          ]
        },
        {
          ...
          visualState: [
            { idx: 0, val: "Left Child (10)", label: "15 > 10 → Go Right" },
            { idx: 1, val: "Node (15)", label: "Found Target!", highlight: "shifted" }
          ]
        }
      ]
    },
...
- Building a heap from an unsorted array by sifting down from index (n/2 - 1) down to 0 takes mathematical O(n) time, NOT O(n log n)!`,
    operationsTable: [
      { operation: "Peek Min/Max", complexity: "O(1)", note: "Direct access to array[0]" },
      { operation: "Insert (offer)", complexity: "O(log n)", note: "Appends to end and bubbles up" },
      { operation: "Extract Min/Max (poll)", complexity: "O(log n)", note: "Replaces root with last leaf and sifts down" },
      { operation: "Build Heap (Heapify)", complexity: "O(n)", note: "Bottom-up sift-down across n/2 non-leaf nodes" },
      { operation: "Remove Arbitrary Object", complexity: "O(n)", note: "Linear scan to find object, then O(log n) sift" }
    ],
    methodsList: [
      {
        name: "offer(E e)",
      ...
      steps: [
        {
          ...
          visualState: [
            { idx: 0, val: 10, label: "Root: Min" },
            { idx: 1, val: 20, label: "Left Child (2*0+1)" },
            { idx: 2, val: 30, label: "Right Child (2*0+2)" }
          ]
        },
        {
          ...
          visualState: [
            { idx: 0, val: 5, label: "New Root Min", highlight: "inserted" },
            { idx: 1, val: 10, label: "Swapped Parent" },
            { idx: 2, val: 30, label: "Right Child" },
            { idx: 3, val: 20, label: "Swapped Down", highlight: "shifted" }
          ]
        }
      ]
    },
...
- If a DFS lands on a node with state == 1, a back-edge cycle is detected!`,
    operationsTable: [
      { operation: "BFS / DFS Traversal", complexity: "O(V + E)", note: "Visits every vertex and edge once" },
      { operation: "Dijkstra (Shortest Path)", complexity: "O((V + E) log V)", note: "Greedy choice using Min-Heap" },
      { operation: "Topological Sort (Kahn's)", complexity: "O(V + E)", note: "Requires DAG (Directed Acyclic Graph)" },
      { operation: "Bellman-Ford", complexity: "O(V * E)", note: "Supports negative edge weights; detects negative cycles" },
      { operation: "Floyd-Warshall", complexity: "O(V³)", note: "All-Pairs Shortest Path 2D/3D dynamic programming" }
    ],
    methodsList: [
      {
      ...
      steps: [
        {
          ...
          visualState: [
            { idx: 0, val: "0 → [1, 2]", label: "Node 0 Neighbors" },
            { idx: 1, val: "1 → [2]", label: "Node 1 Neighbors" },
            { idx: 2, val: "2 → []", label: "Node 2 Neighbors" }
          ]
        },
        {
          ...
          visualState: [
            { idx: 0, val: "Node 0 (Visited)", label: "Level 0" },
            { idx: 1, val: "Node 1 (Queue)", label: "Level 1", highlight: "inserted" },
            { idx: 2, val: "Node 2 (Queue)", label: "Level 1", highlight: "inserted" }
          ]
        }
      ]
    },
...
- Reconstructs sorted array in strictly linear time.`,
    operationsTable: [
      { operation: "Merge Sort", complexity: "O(n log n)", note: "Stable, O(n) extra memory" },
      { operation: "Quick Sort", complexity: "O(n log n) avg / O(n²) worst", note: "In-place, unstable, fast cache performance" },
      { operation: "Heap Sort", complexity: "O(n log n)", note: "In-place (O(1) space), unstable" },
      { operation: "Counting Sort", complexity: "O(n + k)", note: "Non-comparison, stable, integer ranges" },
      { operation: "Insertion Sort", complexity: "O(n) best / O(n²) worst", note: "Stable, in-place, optimal for n <= 32" }
    ],
    methodsList: [
      {
      ...
      steps: [
        {
          ...
          visualState: [
            { idx: 0, val: 12, label: "< Pivot" },
            { idx: 1, val: 18, label: "< Pivot" },
            { idx: 2, val: 25, label: "Pivot Placed", highlight: "inserted" },
            { idx: 3, val: 40, label: "> Pivot" },
            { idx: 4, val: 32, label: "> Pivot" }
          ]
        }
      ]
    },
...
const targetPath = path.join(process.cwd(), "src/data/dataStructuresData.js");
fs.writeFileSync(targetPath, "export const DATA_STRUCTURES = " + JSON.stringify(DATA_STRUCTURES, null, 2) + ";\n", "utf8");
console.log("Successfully generated comprehensive dataStructuresData.js!");
